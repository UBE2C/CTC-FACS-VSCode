...
iSort_unif <- mutate(iSort_df, dates = metadata[[4]], resp.Group = metadata[[3]], oSampleID = metadata[[1]],
                     fSampleID = metadata[[2]], .before = "EpCAM")
...
impute_low_MFI_value = function(data, channel1, channel2, replacement_for_random_sampling = FALSE) {
...
impute_low_MFI_value(niSortU_RvNR, "EpCAM", "PSMA")
...
# Remove samples where the Ac treatment was inconsistent
niSortFilt <- niSortFilt[-grep("ALM66C3-003", niSortFilt$patientID), ]
niSortFilt <- niSortFilt[-grep("ALM68C6-005", niSortFilt$patientID), ]
niSortFilt <- niSortFilt[-grep("ALM67C6-011", niSortFilt$patientID), ]
niSortFilt <- niSortFilt[-grep("ALM81C6-017", niSortFilt$patientID), ]
niSortFilt <- niSortFilt[-grep("ALM75C7-006", niSortFilt$patientID), ]
...
summary_cell_number_stat_median_p <- ggplot(data = filtered_cell_numbers_df, aes(x = responseGroup, y = totalCellNumber, fill = responseGroup)) +
                                    geom_boxplot() +
                                    geom_dotplot(stackdir = "center", binaxis = "y", binwidth = 1, alpha = 0.5, show.legend = FALSE) +
...
# Preparing the full dataset for the cycle wise analysis (cCycle = collection cycle, tCycle = treatment cycle, AcCycle = active Ac treatment cycle)
...
long_CEpCAM_res_filt <- dplyr::filter(.data = long_CEpCAM_res, tCycle < 3)
...
long_CEpCAM_nonres_filt <- dplyr::filter(.data = long_CEpCAM_nonres, tCycle < 3)


# Pairwise comparison
tCycle_NREpCAM_wilcox_p <- ggplot(data = long_CEpCAM_nonres_filt,
                                 aes(x = factor(tCycle), y = log10(MFI_values), fill = factor(tCycle), group = factor(tCycle))) +
    geom_pwc(method = "wilcox_test", label = "p = {p}", ref.group = "0") + #this geom is for pairwise comparison and is amazing!
    geom_boxplot(show.legend = FALSE, position = "dodge") +
    geom_dotplot(show.legend = FALSE, binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5,
...
long_CPSMA_res_filt <- dplyr::filter(.data = long_CPSMA_res, tCycle < 3)
...
long_CPSMA_nonres_filt <- dplyr::filter(.data = long_CPSMA_nonres, tCycle < 3)


# Pairwise comparison
tCycle_NRPSMA_wilcox_p <- ggplot(data = long_CPSMA_nonres_filt,
                                  aes(x = factor(tCycle), y = log10(MFI_values), fill = factor(tCycle), group = factor(tCycle))) +
    geom_pwc(method = "wilcox_test", label = "p = {p}", ref.group = "0") + #this geom is for pairwise comparison and is amazing!
    geom_boxplot(show.legend = FALSE, position = "dodge") +
    geom_dotplot(show.legend = FALSE, binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5,
...
# For EpCAM
long_CEpCAM_filt <- dplyr::filter(.data = long_CEpCAM, tCycle < 3)
...
# For PSMA
long_CPSMA_filt <- dplyr::filter(.data = long_CPSMA, tCycle < 3)
...
# For responders
long_cycles_resp <- dplyr::filter(.data = long_cycles, resp.Group == "Responder" & tCycle < 3)
...
ggsave(filename = paste0("Total_MFI_per_marker_groups_over_treatment_cycles_Resp", "_", script_version, ".png"), tCycle_R_Markers_p,
       device = "png", path = paste0(script_version, "/", "Plots"),
       width = 4000, height = 1500, units = "px", dpi = 320)
rm(tCycle_R_Markers_p)


# For nonresponders
long_cycles_nonresp <- dplyr::filter(.data = long_cycles, resp.Group == "Nonresponder" & tCycle < 3)
