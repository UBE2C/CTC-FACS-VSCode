...
write_csv(x = iSort_unif, file = paste0("Total_unified_index_sorts_with_metadata", "_", script_version, ".csv"))
...
count_CTCs = function(dataframe) {
    ...
    #This line will create a new df containing the unique sampleIDs, the corresponding CTC numbers and the response group
    cellNumbers <- data.frame(sampleID = uniqueIDs, cellNumbers = NA, responseGroup = NA)
    ...
    write_csv(x = cellNumbers, file = paste0("CTC_numbers_per_patient", "_", script_version, ".csv"))
...
count_CTCs(iSort_unif)


#From this point on, I can re-load the saved unified dataframe
iSort_unif <- read_csv(paste0("Total_unified_index_sorts_with_metadata", "_", script_version, ".csv"))
...
#creating an after QC scatter plot to see the result of filtering
...
ggsave(filename = paste0("After_QC_gating_scatterplot", "_", script_version, ".png"), after_QC_scatter_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("Total_raw_cell_numbers_per_response_group", "_", script_version, ".png"),
...
ggsave(filename = paste0("Total_EpCAM_MFI_Resp_v_Nonresp_per_marker_status", "_", script_version, ".png"), EpCAM_RvNR_SE_DP_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("Total_PSMA_MFI_Resp_v_Nonresp_per_marker_status", "_", script_version, ".png"), PSMA_RvNR_SP_DP_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
cycles <- mutate(.data = niSortFilt,
                      cCycle = str_extract(niSortFilt$sampleID, one_or_more(ALNUM) %R% END),
                      tCycle = as.numeric((str_extract(niSortFilt$sampleID, one_or_more(ALNUM) %R% END))) - 1)
...
write_csv(x = tCycle_EpCAM_medians_unif, file = paste0("Updated_analysis/Treatment_cycles_EpCAM_MFI_medians_MADs", "_", script_version, ".csv"))
...
write_csv(x = tCycle_PSMA_medians_unif, file = paste0("Updated_analysis/Treatment_cycles_PSMA_MFI_medians_MADs", "_", script_version, ".csv"))
...
tCycle_R_Markers_p <- ggplot(data = long_cycles_resp,
    ...
    geom_dotplot(show.legend = FALSE, binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5,
                 position = position_dodge(width = 1, preserve = "total")) +
    #geom_pwc(method = "t_test", label = "p = {p}", ref.group = "0") + #this geom is for pairwise comparison and is amazing!
...
ggsave(filename = paste0("Total_MFI_per_marker_groups_over_treatment_cycles_Resp", "_", script_version, ".png"), tCycle_R_Markers_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("Total_MFI_per_marker_groups_over_treatment_cycles_Nonresp", "_", script_version, ".png"), tCycle_NR_Markers_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
