...
lapply(c("tidyverse", "rebus", "flowCore", "flowStats", "flowViz", "ggcyto", "progress", "ggpubr", "ggsci", "rstudioapi", "doSNOW", "foreach"),
       library, character.only = TRUE)

#Define the working directory
path <- c("C:/Work/DKFZ/Alpha_project_data_anal/FACS")
setwd(path)
...
#this function will add dates to each datafrae found in the flowframes (it must be assigned)
#add_dates_to_FFs = function(flow_frame_list) {
    #this part initializes a progress bar for the function with the choosen settings
#    progressBar <- progress_bar$new(format = "(:spin) [:bar] :percent [Elapsed time: :elapsedfull || Estimated time remaining: :eta]",
#                                    total = length(flow_frame_list),
#                                    complete = "=",
#                                    incomplete = "-",
#                                    current = ">",
#                                    clear = FALSE,
#                                    width = 100)
    #this loop will add a new column to each dataframe of the flowframe and fills it with the date, which belongs to the said FF
#    date <- c()
#    for(e in seq_along(flow_frame_list)) {
#        progressBar$tick() #this will lunch the progress bar
#        
#        date[e] <- str_extract(names(flow_frame_list)[e], one_or_more(ASCII_ALNUM) %R% one_or_more(PUNCT) %R% one_or_more(ASCII_ALNUM)
#                               %R% one_or_more(PUNCT) %R% one_or_more(ASCII_ALNUM)) #extracts the dates
#        flow_frame_list[[e]]@exprs <- cbind(flow_frame_list[[e]]@exprs, date)
#        flow_frame_list[[e]]@exprs[, ncol(flow_frame_list[[e]]@exprs)] <- date[e]
#        colnames(flow_frame_list[[e]]@exprs)[ncol(flow_frame_list[[e]]@exprs)] <- "Date"
#        
#        
#    }
#    
#    return(flow_frame_list)
#}
#ctrlFFs <- add_dates_to_FFs(ctrlFFs)
#NOTE: this function will change all the MFI values to character instead of numeric, braking everything adter this
#if you need to use it, trnaform every MFI back to numeric
...
#I will clean the colnames and subset ctrlFF_dfs to get only the fluorophore channels
...
ctrlFF_dfs_Fluo$EpCAM <- as.numeric(ctrlFF_dfs_Fluo$EpCAM)
ctrlFF_dfs_Fluo$PSMA <- as.numeric(ctrlFF_dfs_Fluo$PSMA)
...
#I will prep the ctrlMean_df for the normalization by adding the sort dates to the
#df in a separate column
ctrlDates<- str_extract(rownames(ctrlMedian_df), START %R% one_or_more(ASCII_ALNUM) %R% "-" 
...
df_colnames <- c("FSC.A", "FSC.H", "FSC.W", "SSC.A", "SSC.H",
                 "SSC.W", "EpCAM", "DAPI_CD45", "Caspase_3.7", "PSMA", "Dates")
colnames(ctrlMedian_df) <- df_colnames
head(ctrlMedian_df)

ctrlMedian_Fluo <- ctrlMedian_df[,c(7, 10, 11)]
...
ctrlMedian_Fluo <- dplyr::select(ctrlMedian_Fluo, EpCAM, PSMA, Dates)
...
ctrlMedian_dist_box_p <- ggplot(data = long_ctrlMed_F,
                                    ...
                                    labs(x = expression("Channels"), y = expression("log"[10]*" median MFI values")) +
                                    theme_classic()
...
ctrlMedian_dist_dens_p <- ggplot(data = long_ctrlMed_F,
                                aes(x = MFI_medians, fill = Channels)) +
                                #scale_x_continuous(limits = c(0, 6000)) +
                                geom_density(alpha = 0.5) +
                                scale_fill_tron() +
                                theme_classic()
...
#plotting the Grand Median and MADM values on a barchart with the errorbars
...
rm(ctrlFF_dfs_Fluo, ctrlFF_ind_MAD, ctrlFluo_total_MAD, ctrlFluo_GrMedian, ctrlFluo_sum_stat, ctrlMedian_Fluo, long_ctrlMed_F, ctrlFluo_MADM)
...
#    new_spec_names <- c()
#    new_spec_names[e] <- str_extract(names(iSorts)[e], "Specimen*.*")
#    print(new_spec_names)
#}
...
fix_sample_name_in_FFs = function(data, incorrect_name_pattern) {
    ...
    specimen_names <- list()
    for(e in seq_along(data)) {
        specimen_names[[e]] <- str_extract(keyword(data[[e]])$FIL, "Specimen*.*") 
    }
    ...
    modif_data <- data
    for(e in seq_along(data)) {
        if(str_detect(keyword(data[[e]])$"$FIL", incorrect_name_pattern) == TRUE) {
            incorrect_lst_positions[e] <- e
            keyword(modif_data[[e]])$"$FIL" <- specimen_names[[e]]
        } else {
            #do nothing
        }
    }
...
#than the colname order of the index flow frames so I made this fucntion
...
#INX_ALM83C8-001-3. The TUBE NAME is incorrectly recorded as INX_ALM83C8-001-2, and has to be fixed
#this function looks for the incorrect name (or a part of it) and replaces the sample names which contains this
#pattern with the correct name
correct_FACS_exp_names = function(data, incorrect_name_pattern) {
    #first, create a list with the sample names found in the list of expereiments/samples
    df_name_lst <- list()
    for(e in seq_along(data)){
        df_name_lst[[e]] <- data[[e]]$name
    }
    names(df_name_lst) <- names(data)
    
    #this line will look for the pattern which is in the incorrect sample names (e.g: the experiments were incorrectly named as negative
    #leukocyte controls) and stores these in a new list
    df_names_to_replace <- lapply(df_name_lst, grep, pattern = incorrect_name_pattern)
    
    #This is a control loop for the function. If there are incorrect sample names based on the pattern the function will continue and they
    #will be replaced. Otherwise the fucntion will stop.
    continue <- FALSE
        for(e in seq_along(df_names_to_replace)){
        if(length(df_names_to_replace[e]) != 0){
            continue <- TRUE
        } else {
            continue <- FALSE
        }
    ...
    if(continue == TRUE) {
        #this loop will use the correct experimental group names (flowframe list names) to extract the proper sample names
        ...
        for(e in seq_along(df_name_lst)) {
            df_names_replace_with[[e]] <- str_extract(names(df_name_lst[e]), "Specimen*.*")
        }
        
        #this loop will first look at each flowframe and checks if the same flowframe in the names_to_replace has a length of larger than 0
        #if yes, (which means that there are samples in this flowframe with the incorrect names) it replaces the sample names in those
        #flowframe with the corresponding correct names found in the names_to_replace_with list
        for(e in seq_along(df_name_lst)) {
            if(length(df_names_to_replace[e]) != 0) {
                data[[e]]$name <- str_replace_all(df_name_lst[[e]], unique(df_name_lst[[e]]), df_names_replace_with[[e]])
            } else {
                #do nothing
            }
        }
        ...
        message("No incorrect sample name has been found based on the give pattern. The function will stop.")
        break
    }
...
iSort_dfs <- correct_FACS_exp_names(iSort_dfs, "leukocyte")
...
#this snipet substracts the sample IDs 
#important note: there a too many naming mistakes in here, so I had to write
#super complicated srting matching pattern. Leave the pattern order, otherwise it won't work
...
#this part loads the file names separated by response group so the response can be substracted 
nonResp <- list.files(path = paste0(path, "/", "Non-responder indexes"))
...
Resp <- list.files(path = paste0(path, "/", "Responder indexes"))
...
Unkn <- list.files(path = paste0(path, "/", "Unclassified indexes"))
...
#additionally, I realized that the sample naming is fixed for the sorted index files
...
#I noticed that one of the sequenced sample was not labeled and for some reason and I can't seem to fix it in the loop
#therefore I will fix it by hand
...
#Dates <- c()
#for (e in seq_along(iSorts)) {
#  for (i in seq_along(iSort_df$Name)) {
#    if (str_detect(unlist(keyword(iSorts[[e]], "$FIL")), iSort_df$Name[i]) == TRUE) {
#      Dates[i] <- str_extract(names(iSorts)[e], START %R% one_or_more(ALNUM) %R%
#                                "-" %R% one_or_more(ALNUM) %R%
#                                "-" %R% one_or_more(ALNUM))
#   } else {
#      #do nothing
...
#  }
#}
#this slpit loop is much more efficient at doing the same thing as the main loop
...
#Note:don't use grep, grepl or str_detect, because for some reason it does not work well with
#the brackets
...
#database
metadat <- list(sampleID, resp.Group, Dates)
rm(sampleID, resp.Group, Dates, Resp, nonResp, Unkn)

iSort_unif <- mutate(iSort_df, sampleID = metadat[[1]],
                     resp.Group = metadat[[2]], dates = metadat[[3]]) 

#I will save the generated dataframe
write_csv(x = iSort_unif, file = paste0("Unified_index_sorts_with_metadata", "_", script_version, ".csv"))
write_lines(x = rownames(iSort_unif), file = paste0("Unified_index_sorts_with_metadata_rownames", "_", script_version, ".txt"))
iSortU_rownames <- rownames(iSort_unif)



#From this point on, I can re-load the saved unified dataframe
iSort_unif <- read.csv(paste0("Unified_index_sorts_with_metadata", "_", script_version, ".csv"))
iSortU_rownames <- read_lines(file = paste0("Unified_index_sorts_with_metadata_rownames", "_", script_version, ".txt"))
rownames(iSort_unif) <- iSortU_rownames
rm(iSortU_rownames)


##I will filter the main dataset to match the sequenced samples only

#First I will load the latest metadata used during the scRNA-seq analysis
seq_meta <- read.csv("Corrected_seq_metadata_for_seurat.csv", sep = ";")
head(seq_meta)

#I will match the sampleIDs with the seq-sampleIDs by removing the hypens
iSort_unif$sampleID <- str_replace_all(iSort_unif$sampleID, "-" %|% SPACE %|% "\\(" %|% "\\)", "")
head(iSort_unif)


#I will match the sortIDs with the seqIDs to see if they are matching properly
unique(seq_meta$patientID) %in% unique(iSort_unif$sampleID)



unique(seq_meta$patientID[!seq_meta$patientID %in% iSort_unif$sampleID])




##I will trim the FACS data to exactly match the sequencing data
#Note:to do a proper position matched trimming, I will need to do this in two steps.




#Step1: pre-trimming and data some data fixing
#I will trim the index sorts based on the match between the sorts and the sequenced cells
#this will not give me an exact match as I will have all the cells form the sorts rather than the ones
#actually sequenced
iSort_unif <- iSort_unif[iSort_unif$sampleID %in% seq_meta$patientID, ]
head(iSort_unif)
#Note: iSort_Unif from this point on will refer to the trimmed data, so the script is more simple to run!


#this loop will do the normalization based on date matching (it should be accurate as the 
#samples had uniqe IDs so their date macthing should be correct...)
...
for (i in seq_along(ctrlMedian_df$Dates)) {
  ...
  for (e in seq_along(tmp)) {
    norm_iSortu[tmp[e], 2:5] <- iSort_unif[tmp[e], 2:5]/ctrlMedian_df[i, 7:10]
  }
  
}
head(norm_iSortu)
norm_iSortu <- norm_iSortu[, -c(3, 4)]
head(norm_iSortu)


#As I'm mostly interested in the resp vs nonresp, I will create a df where the unknown
#resp.Group is removed
#Not normalized index sorts
#iSortU_C_RvNR <- iSortu_clean[!grepl("Unknown" ,iSortu_clean$resp.Group), ]
#iSortU_C_RvNR <- iSortU_C_RvNR[!grepl("leukocyte", iSortU_C_RvNR$resp.Group), ]

#normalized index sorts
...
#niSortFilt <- niSortU_RvNR[niSortU_RvNR$EpCAM > 0 & niSortU_RvNR$PSMA > 0, ]


#This function will imput the mean of 2 randomly selected MFI values from the top10 lowest MFI values
imput_low_MFI_value = function(data, channel1, channel2, replacement_for_random_sampling = FALSE) {
    ...
    #for some reason subsetting creates lists which I will unlist into vectors
    channel1_values_above_zero <- unlist(channel1_values_above_zero)
    channel2_values_above_zero <- unlist(channel2_values_above_zero)
    ...
    for (e in 1:nrow(imputed_df)) {
        ...
        #The if statement for cahnnel 2
    ...
    #creating a namw for the new object based on the name of the original dataframe
    old_object_name <- deparse(substitute(data))
...
#I will mark the trimmed FACS data with the marker status for the seq data
niSortU_RvNR_imp_meta_for_seq <- niSortU_RvNR_imputed

niSortU_RvNR_imp_meta_for_seq$EvP_status <- NA
for (e in 1:nrow(niSortU_RvNR_imp_meta_for_seq)) {
    if (log10(niSortU_RvNR_imp_meta_for_seq$EpCAM[e]) < -1) {
        niSortU_RvNR_imp_meta_for_seq$EvP_status[e] <- "SP"
    } else if (log10(niSortU_RvNR_imp_meta_for_seq$PSMA[e]) < -1.5) {
        niSortU_RvNR_imp_meta_for_seq$EvP_status[e] <- "SE"
    } else {
        niSortU_RvNR_imp_meta_for_seq$EvP_status[e] <- "DP"
    }
}


#I must do position matching otherwise this will be good for nothing
#This function should position match the trimmed data and assign it into a new df
position_matching = function(trimmed_dataset, position_dataset, XLoc_col_index, YLoc_col_index) {
    #first I will extract the full sample names form the trimmed dataset, so I can get the positions for these from the
    #position dataset
    trimmed_sample_names_lst <- str_extract_all(rownames(trimmed_dataset), ".*.fcs")
    trimmed_sample_names_lst <- do.call(rbind, trimmed_sample_names_lst)
    
    #Next, this loop will fetch the position columns form the position datasetbased on matching sample names
    #it is very important that the value and fixed arguments for grep must be TRUE 
    sort_positions <- list()
    for(e in 1:nrow(unique(trimmed_sample_names_lst))) {
        position <- grep(unique(trimmed_sample_names_lst)[e, 1], names(position_dataset), fixed = TRUE, value = TRUE)
        sort_positions[[position]] <- position_dataset[[position]][XLoc_col_index:YLoc_col_index]
        
    }
    
    #Next the position list will be converted into a df. A new ID column will be added and the position columns will be
    #renamed
    sort_positions_df <- do.call(rbind, sort_positions)   
    sort_positions_df <- mutate(sort_positions_df, sampleID = trimmed_dataset$sampleID, .before = XLoc)
    colnames(sort_positions_df) <- c("sampleID", "plate_rows", "plate_cols")
    
    #this if statement should ensure that the input and output lengths are the same
    if(nrow(sort_positions_df) == nrow(trimmed_dataset)) {
        assign("position_matched_iSorts", sort_positions_df, envir = .GlobalEnv)
        message("Position matching succesful, a new object was created: position_matched_iSorts")
        
    } else {
        message("Something went wrong during the position matching as the output lenght does not match the length of the trimmed dataset.
                Please check the function script and the provdied arguments.")
        break
    }
...
position_matching(niSortU_RvNR_imp_meta_for_seq, iSort_dfs, 13, 14)
head(position_matched_iSorts)


#I will unify the pre-trimmed and imputed dataset and the position matched dataset for seq and will save the new unif. df
unif_position_matched_and_trimmed_imp_meta <- cbind(niSortU_RvNR_imp_meta_for_seq, position_matched_iSorts[, 2:3])

write_csv(x = unif_position_matched_and_trimmed_imp_meta,
          file = paste0("Position_matched_trimmed_imputed_metadata_with_marker_status_for_seq", "_", script_version, ".csv"))
rm(niSortU_RvNR_imp_meta_for_seq, position_matched_iSorts, unif_position_matched_and_trimmed_imp_meta)




## Step 2: I used the position matched data together with the seq data to identify each sorted cell which was actually sequenced
##now I can do the actual position matched trimming which will give me FACS data, reflecting the RNA seq

#I will load the .csv file, containing the matched FACS-SEQ data (this file was created outside of R, using Excel)
sample_matched <- read.csv("Sample_matched_trimming_metadata.csv", sep = ";")
head(sample_matched)


#I will name the rows according to the non-matched data
rownames(sample_matched) <- rownames(niSortU_RvNR)
head(sample_matched)


#I can finally create the properly trimmed and matched dataset
#NOTE: from now on, the niSortU_RvNR and niSortU_RvNR_imputed will represent the matched and trimmed dataset
niSortU_RvNR <- niSortU_RvNR[!is.na(sample_matched$sampleID), ]
niSortU_RvNR_imputed <- niSortU_RvNR_imputed[!is.na(sample_matched$sampleID), ]




##Let's try to visualize some parameters with the normalized samples and filter the data as needed

#first, transform the dataframe to a long format
niSortU_RvNR <- dplyr::select(niSortU_RvNR,
                               sampleID, resp.Group, EpCAM, PSMA)
long_niSortU_RvNR <- pivot_longer(niSortU_RvNR, cols = c(EpCAM, PSMA),
                               names_to = "Channels", values_to = "MFI_values")

#visualizing the fluorophore channels
ggplot(data = long_niSortU_RvNR,
       aes(fill = resp.Group, x = Channels, 
           y = log10(MFI_values))) +
  #geom_boxplot() +
  geom_dotplot(show.legend = FALSE, stackdir = "center", binaxis = "y",
               binwidth = 0.05, alpha = 0.5) +
    facet_wrap(vars(resp.Group)) +
  theme_classic()
...
ggsave(filename = paste0("QC_gating_scatterplot_trimmed", "_", script_version, ".png"), QC_scatter_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(QC_scatter_p)


#I will also filter the data to remove suspiciously low and high values
...
#I will re-plot the filtered data
...
ggsave(filename = paste0("after_QC_gating_scatterplot_trimmed", "_", script_version, ".png"), after_QC_scatter_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(after_QC_scatter_p)


#creating a bar chart showing the total cell numbers sorted / response group
...
ggsave(filename = paste0("Trimmed_raw_cell_numbers_per_response_group", "_", script_version, ".png"),
...
ggsave(filename = paste0("Marker_status_scatterplot_trimmed", "_", script_version, ".png"), QC_EvP_status_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(QC_EvP_status_p)


#Save the normalized, imputed and filtered position matched dataframe
write_csv(x = niSortFilt, file = paste0("Norm_matched_trimmed_imput_filt_indexDataframe", "_", script_version, ".csv"))




##In-depth marker analysis
...
write_csv(unif_cell_numbers_per_marker, file = paste0("Trimmed_raw_cell_numbers_with_marker_dist", "_", script_version, ".csv"))
write_csv(unif_cell_numbers_per_marker_percent, file = paste0("Trimmed_raw_cell_number_percent_with_marker_dist.csv", "_", script_version, ".png"))


#Visualizing raw cell numbers based on marker distribution (thx bing!)
...
#long_unif_cell_numbers_per_marker$resp.Group <- factor(
#    long_unif_cell_numbers_per_marker$resp.Group,
#    levels = c("Responder", "Nonresponder")  # Change the order for displaying
#)
#print(long_unif_cell_numbers_per_marker)
...
ggsave(filename = paste0("Raw_cell_numbers_per_marker_trimmed", "_", script_version, ".png"), raw_cell_numbers_per_marker_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
#long_unif_cell_numbers_per_marker_percent$resp.Group <- factor(
#    long_unif_cell_numbers_per_marker_percent$resp.Group,
#    levels = c("Responder", "Nonresponder")  # Change the order for displaying
#)
#print(long_unif_cell_numbers_per_marker_percent)
...
ggsave(filename = paste0("Cell_number_percenteges_per_marker_trimmed", "_", script_version, ".png"), marker_percentages_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
#I will look at the marker distribution between response groups based on MFI. First I will create a responder specific df
long_niSortFilt_resp <- dplyr::filter(long_niSortFilt, resp.Group == "Responder")
...
ggsave(filename = paste0("Marker_status_on_responders_trimmed", "_", script_version, ".png"), marker_status_resp_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("Marker_status_on_nonresponders_trimmed", "_", script_version, ".png"), marker_status_nonresp_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
marker_status_unif_p <- ggplot(long_niSortFilt,
    ...
    geom_dotplot(show.legend = FALSE, position = position_dodge(width = 1, preserve = "total"),
                 binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5) +geom_pwc(method = "wilcox_test", label = "p = {p}") +
    scale_fill_tron() +
...
ggsave(filename = paste0("Marker_status_on_both_responsegroups_trimmed", "_", script_version, ".png"), marker_status_unif_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("Total_EpCAM_MFI_Resp_v_Nonresp_trimmed", "_", script_version, ".png"), EpCAM_RvNR_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(EpCAM_RvNR_p)


#Visualizing EpCAM channel between reponse groups broken down to marker status
long_niSortFilt_EpCAM_SE_DP <- dplyr::filter(long_niSortFilt, EvP_status == "SE" |EvP_status == "DP")


EpCAM_RvNR_SE_DP_p <- ggplot(long_niSortFilt_EpCAM_SE_DP,
                             aes(x = EvP_status, y = log10(MFI_values), fill = resp.Group)) +
    geom_boxplot(show.legend = TRUE, position = position_dodge(width = 1, preserve = "single")) +
    geom_dotplot(show.legend = FALSE, position = position_dodge(width = 1, preserve = "total"),
                 binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5) +
    geom_pwc(method = "wilcox_test", label = "p = {p}") +
    scale_fill_tron() +
    scale_x_discrete(labels = c("Double positive","EpCAM positive")) +
    #geom_segment(aes(x = "DP", xend = "DP", y = 1.3, yend = 1.3, group = resp.Group), 
    #             col = "black", linewidth = 1, position = position_dodge(width = 1)) +
    #geom_segment(aes(x = "SE", xend = "SE", y = 1.3, yend = 1.3, group = resp.Group), 
    #             col = "black", linewidth = 1, position = position_dodge(width = 1)) +
    #stat_compare_means(label.y = 1.4, label.x = 1.4) +
    labs(x = "Marker status", y = expression("EpCAM log"[10]*" MFI values"), fill = "Response Group") +
    ggtitle("EpCAM MFI and marker status comparison between response groups") +
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5))
print(EpCAM_RvNR_SE_DP_p)

ggsave(filename = paste0("Total_EpCAM_MFI_Resp_v_Nonresp_per_marker_status_trimmed", "_", script_version, ".png"), EpCAM_RvNR_SE_DP_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(EpCAM_RvNR_SE_DP_p)
...
ggsave(filename = paste0("Total_PSMA_MFI_Resp_v_Nonresp_trimmed", "_", script_version, ".png"), PSMA_RvNR_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(PSMA_RvNR_p)


#Visualizing PSMA channel between reponse groups broken down to marker status
long_niSortFilt_PSMA_SP_DP <- dplyr::filter(long_niSortFilt, EvP_status == "SP" |EvP_status == "DP")


PSMA_RvNR_SP_DP_p <- ggplot(long_niSortFilt_PSMA_SP_DP,
                             aes(x = EvP_status, y = log10(MFI_values), fill = resp.Group)) +
    geom_boxplot(show.legend = TRUE, position = position_dodge(width = 1, preserve = "single")) +
    geom_dotplot(show.legend = FALSE, position = position_dodge(width = 1, preserve = "total"),
                 binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5) +
    geom_pwc(method = "wilcox_test", label = "p = {p}") +
    scale_fill_tron() +
    scale_x_discrete(labels = c("Double positive","PSMA positive")) +
    #geom_segment(aes(x = "DP", xend = "DP", y = 1.3, yend = 1.3, group = resp.Group), 
    #             col = "black", linewidth = 1, position = position_dodge(width = 1)) +
    #geom_segment(aes(x = "SP", xend = "SP", y = 1.3, yend = 1.3, group = resp.Group), 
    #             col = "black", linewidth = 1, position = position_dodge(width = 1)) +
    #stat_compare_means(label.y = 1.4, label.x = 1.4) +
    labs(x = "Marker status", y = expression("PSMA log"[10]*" MFI values"), fill = "Response Group") +
    ggtitle("PSMA MFI and marker status comparison between response groups") +
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5))
print(PSMA_RvNR_SP_DP_p)

ggsave(filename = paste0("Total_PSMA_MFI_Resp_v_Nonresp_per_marker_status_trimmed", "_", script_version, ".png"), PSMA_RvNR_SP_DP_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(PSMA_RvNR_SP_DP_p)




##Exploring and visualizing the EpCAM and PSMA MFI cahnges over the cycles


#Preparing the full dataset for the cyclewise analysis (cCycle = collection cycle, tCycle = treatment cycle)
cycles <- mutate(.data = niSortFilt,
                      cCycle = str_extract(niSortFilt$sampleID, "." %R% END),
                      tCycle = as.numeric((str_extract(niSortFilt$sampleID, "." %R% END))) - 1)
...
#I will also calculate the MAD values for each cycel
#for responders
tCycle_REpCAM_mad <- c()
for(i in min(long_CEpCAM_res$tCycle):max(long_CEpCAM_res$tCycle)) {
...
#I decided not look at treatmetn cycles which did not have at least 10 cells
...
write_csv(x = tCycle_EpCAM_medians_unif, paste0("Updated_analysis/Treatment_cycles_EpCAM_MFI_medians_MADs_trimmed", "_", script_version, ".csv"))
...
ggsave(filename = paste0("EpCAM_MFI_over_treatment_cycles_Responders_wilcox_trimmed", "_", script_version, ".png"), tCycle_REpCAM_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("EpCAM_MFI_over_treatment_cycles_Nonresponders_wilcox_trimmed", "_", script_version, ".png"), tCycle_NREpCAM_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
write_csv(x = tCycle_PSMA_medians_unif, paste0("Updated_analysis/Treatment_cycles_PSMA_MFI_medians_MADs_trimmed", "_", script_version, ".csv"))
...
ggsave(filename = paste0("PSMA_MFI_over_treatment_cycles_Responders_wilcox_trimmed", "_", script_version, ".png"), tCycle_RPSMA_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("PSMA_MFI_over_treatment_cycles_Nonresponders_wilcox_trimmed", "_", script_version, ".png"), tCycle_NRPSMA_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("EpCAM_MFI_over_treatment_cycles_Resp_vs_Nonresp_wilcox_trimmed", "_", script_version, ".png"), tCycle_RvNREpCAM_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
ggsave(filename = paste0("PSMA_MFI_over_treatment_cycles_Resp_vs_Nonresp_wilcox_trimmed", "_", script_version, ".png"), tCycle_RvNRPSMA_wilcox_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
...
#marker based comparison per response group
#for responders
long_cycles_resp <- dplyr::filter(.data = long_cycles, resp.Group == "Responder" & tCycle != 4)

tCycle_R_Markers_p <- ggplot(data = long_cycles_resp,
                             aes(x = factor(tCycle), y = log10(MFI_values), fill = factor(EvP_status))) +
    geom_boxplot(show.legend = TRUE, position = position_dodge(width = 1, preserve = "single")) +
    geom_dotplot(show.legend = FALSE, binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5,
                 position = position_dodge(width = 1, preserve = "total")) +
    #geom_pwc(method = "t_test", label = "p = {p}") + #this geom is for pairwise comparison and is amazing!
    scale_fill_tron(labels = c("Double positive", "EpCAM positive", "PSMA positive")) +
    #scale_fill_manual() +
    stat_compare_means(label.y = 1.2, label.x = 1.2, method = "kruskal.test") +
    labs(x = "Treatment cycles", y = expression("Total log"[10]*" MFI values"), fill = "Marker status") +
    ggtitle("Total MFI comparison between treatment cycles - Responders") +
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5))
print(tCycle_R_Markers_p)

ggsave(filename = paste0("Total_MFI_per_marker_groups_over_treatment_cycles_Resp_trimmed", "_", script_version, ".png"), tCycle_R_Markers_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(tCycle_R_Markers_p)

#for nonresponders
long_cycles_nonresp <- dplyr::filter(.data = long_cycles, resp.Group == "Nonresponder" & tCycle != 4)

tCycle_NR_Markers_p <- ggplot(data = long_cycles_nonresp,
                             aes(x = factor(tCycle), y = log10(MFI_values), fill = factor(EvP_status))) +
    geom_boxplot(show.legend = TRUE, position = position_dodge(width = 1, preserve = "single")) +
    geom_dotplot(show.legend = FALSE, binaxis = "y", stackdir = "center", binwidth = 0.05, alpha = 0.5,
                 position = position_dodge(width = 1, preserve = "total")) +
    ...
    stat_compare_means(label.y = 1.2, label.x = 1.2, method = "kruskal.test") +
    labs(x = "Treatment cycles", y = expression("Total log"[10]*" MFI values"), fill = "Marker status") +
    ggtitle("Total MFI comparison between treatment cycles - Nonresponders") +
    theme_classic() +
    theme(plot.title = element_text(hjust = 0.5))
print(tCycle_NR_Markers_p)

ggsave(filename = paste0("Total_MFI_per_marker_groups_over_treatment_cycles_Nonresp_trimmed", "_", script_version, ".png"), tCycle_NR_Markers_p,
       device = "png", path = "Updated_analysis/Plots/",
       width = 3000, height = 1500, units = "px")
rm(tCycle_NR_Markers_p)
