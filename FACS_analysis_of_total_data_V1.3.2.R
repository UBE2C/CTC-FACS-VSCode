...
setwd(path)
FACS_files <- c("/all_fcs_csv_files")
...
grep("2022-04-13", names(iSorts))
for (i in c(13:21)) {
  print(paste0(i, " ", keyword(iSorts[[i]])$"$DATE", " ", keyword(iSorts[[i]])$"$FIL", " ", keyword(iSorts[[i]])$"$BTIM"))
}

#Sample batch 1 - correction
flowCore::keyword(iSorts[[16]])$"$FIL" <- "Specimen_001_INX_ALM76C6-003-1_strip_1_001_007.fcs"
flowCore::keyword(iSorts[[17]])$"$FIL" # Correct
flowCore::keyword(iSorts[[18]])$"$FIL" <- "Specimen_001_INX_ALM80C8-004-2 strip 1_002_006.fcs"

#Sample batch 2 - check
grep("ALM83C8", names(iSorts))
names(iSorts)[[48]]
flowCore::keyword(iSorts[[48]])$"$FIL"

for (i in c(47:59)) {
  print(paste0(i, " ", keyword(iSorts[[i]])$"$DATE", " ", keyword(iSorts[[i]])$"$FIL", " ", keyword(iSorts[[i]])$"$BTIM"))
}

#Sample batch 2 - correction
flowCore::keyword(iSorts[[48]])$"$FIL" <- "Specimen_001_INX_ALM83C8-001-3_str_1_001_005.fcs"

#Sample batch 3 - check
...
grep("2022-05-24", names(iSorts))
for (i in c(153:156)) {
  print(paste0(i, " ", keyword(iSorts[[i]])$"$DATE", " ", keyword(iSorts[[i]])$"$FIL", " ", keyword(iSorts[[i]])$"$BTIM"))
}

#Sample batch 3 - correction
...
#Sample batch 4 - check
grep("leukocyte", names(iSorts))
names(iSorts)[grep("leukocyte", names(iSorts))]

for (i in c(21:36)) {
  print(paste0(i, " ", keyword(iSorts[[i]])$"$DATE", " ", keyword(iSorts[[i]])$"$FIL", " ", keyword(iSorts[[i]])$"$BTIM"))
}

#Sample batch 4 - correction
flowCore::keyword(iSorts[[22]])$"$FIL" <- "Specimen_001_INX_ALM76C6-003-2_str1_001.fcs"
flowCore::keyword(iSorts[[23]])$"$FIL" <- "Specimen_001_INX_ALM76C6-003-2_str2_002.fcs"
flowCore::keyword(iSorts[[24]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str1_001.fcs"
flowCore::keyword(iSorts[[25]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str2_002.fcs"
flowCore::keyword(iSorts[[26]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str3_003.fcs"
flowCore::keyword(iSorts[[27]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str4_004.fcs"
flowCore::keyword(iSorts[[28]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str5_005.fcs"
flowCore::keyword(iSorts[[29]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str6_006.fcs"
flowCore::keyword(iSorts[[30]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str7_007.fcs"
flowCore::keyword(iSorts[[31]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str8_008.fcs"
flowCore::keyword(iSorts[[32]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str9_009.fcs"
flowCore::keyword(iSorts[[33]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str10_010.fcs"
flowCore::keyword(iSorts[[34]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str11_011.fcs"
flowCore::keyword(iSorts[[35]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_str12_012.fcs"
flowCore::keyword(iSorts[[36]])$"$FIL" <- "Specimen_001_INX_ALM66C5-008-1_96wp_005.fcs"


# This loop pulls the corrected sort names from the FFs
correct_sort_names = function(FF_list) {
  ## Declare variables
  correct_sort_names <- vector(mode = "character", length = length(FF_list))
  original_sort_dates <- vector(mode = "character", length = length(FF_list))
  converted_sort_dates <- vector(mode = "character", length = length(FF_list))
  formatted_sort_dates <- vector(mode = "character", length = length(FF_list))

  ## Replace the sort names with the corret ones

  #Extract the corrected sort names from the $FIL slot
  for (i in seq_along(FF_list)) {
    correct_sort_names[i] <- flowCore::keyword(iSorts[[i]])$"$FIL" 
  }

  #Extract the sort dates from the $DATE slot and format them
  for (e in seq_along(FF_list)) {
    original_sort_dates <- flowCore::keyword(iSorts[[e]])$"$DATE"
    converted_sort_dates <- as.Date(original_sort_dates, format = "%d-%b-%Y")
    formatted_sort_dates[e] <- format(converted_sort_dates, format = "%Y-%m-%d")
  }

  #Paste the Dates and correct names togeather and replace theincorrect ones
  names(FF_list) <- paste0(formatted_sort_dates, " ", correct_sort_names)
    
  ## Return the modified list
  return(FF_list)
}
iSorts <- correct_sort_names(iSorts)
...
# Convert the dataframe list to a large dataframe
...
fixed_ID <- str_replace_all(fixed_ID, pattern = "\\(A\\)",
                            replacement = "A")
# IMPORTANT NOTE: this typo happened during sorting
...
rm(dates_df, CTC_counts, fixed_ID, metadata, sampleID, resp.Group, Dates, Resp, nonResp, Unkn)
#################################################################   End Section  ##################################################################
...
if (!exists("iSort_unif", envir = .GlobalEnv)) {
    ...
    if (file.exists(paste0(script_version, "/", "Total_unified_index_sorts_with_metada
    ta", "_", script_version, ".csv"))) {

        iSort_unif <- read_csv(paste0(script_version, "/", "Total_unified_index_sorts_with_metadata", "_", script_version, ".csv"))
        ...
        paste0("Total_unified_index_sorts_with_metadata", "_", script_version, ".csv"), "\n",
        "was not found in the current script version directory.")
...
## Run the normalization based on date matching (it should be accurate as the 
## samples had unique IDs so their date matching should be correct...)
...
count_filtered_cell_numbers = function(filtered_data_frame) {
    ...
    sampleIDs <- filtered_data_frame$sampleID
    ...
    for (i in seq_along(uniqueSampleIDs)) {
        ...
        filtered_cell_numbers[i, 2] <- unique(dplyr::filter(filtered_data_frame, sampleID == uniqueSampleIDs[i])$resp.Group)
        filtered_cell_numbers[i, 3] <- nrow(dplyr::filter(filtered_data_frame, sampleID == uniqueSampleIDs[i]))
...
summary_cell_number_stats_p <- ggplot(data = filtered_cell_numbers_df, aes(x = responseGroup, y = totalCellNumber, fill = responseGroup)) +
                                ...
                                geom_pwc(method = "t_test", label = "p = {p}", y.position = 14) + #this geom is for pairwise comparison and is amazing!
                                #scale_y_continuous(limits = c(0, 250), breaks = seq(0, 250, 10)) +
                                scale_x_discrete(labels = c("Nonresponder", "Responder")) +
                                scale_fill_tron(labels = c("Nonresponder", "Responder")) +
...
summary_cell_number_stat_median_p <- ggplot(data = filtered_cell_numbers_df, aes(x = responseGroup, y = totalCellNumber, fill = responseGroup)) +
                                    ...
                                    geom_pwc(method = "wilcox_test", label = "p = {p}", y.position = 45) + #this geom is for pairwise comparison and is amazing!
                                    scale_x_discrete(labels = c("Nonresponder", "Responder")) +
                                    scale_fill_tron(labels = c("Nonresponder", "Responder")) +
...
raw_cell_numbers_per_marker_p <- ggplot(data = long_unif_cell_numbers_per_marker,
    ...
    #geom_pwc(method = "wilcox_test", label = "p = {p}") + #this geom is for pairwise comparison and is amazing!
    #scale_y_continuous(limits = c(0, 70), breaks = seq(0, 70, 10)) +
    scale_x_discrete(labels = c("EpCAM+PSMA+", "EpCAM+PSMA-", "EpCAM-PSMA+")) +
    scale_fill_tron(labels = c("Nonresponder", "Responder")) +
    #scale_fill_manual(values = c("#00BFC4", "#F8766D"), labels = c("Responder", "Nonresponder")) +
...
marker_cont_table_DP <- data.frame(observed_count = unif_cell_numbers_per_marker[, 1],
                                expected_count = rbind(c(sum(unif_cell_numbers_per_marker[, 1]) * 0.5), c(sum(unif_cell_numbers_per_marker[, 1]) * 0.5)))


# Contingency table for the EpCAM+PSMA- population
# NOTE: the table will contain the count data and the expected count data
# if the distribution is 50-50, so there is no difference between the response groups
marker_cont_table_E <- data.frame(observed_count = unif_cell_numbers_per_marker[, 2],
...
# NOTE: the table will contain the count data and the expected count data
# if the distribution is 50-50, so there is no difference between the response groups
marker_cont_table_P <- data.frame(observed_count = unif_cell_numbers_per_marker[, 3],
...
marker_status_resp_p <- ggplot(long_niSortFilt_resp,
    ...
    scale_x_discrete(labels = c("DP" = "EpCAM+PSMA+", "SE" = "EpCAM+PSMA-", "SP" = "EpCAM-PSMA+")) +
...
# Visualizing EpCAM channel between response groups broken down to marker status
...
EpCAM_RvNR_SE_DP_p <- ggplot(long_niSortFilt_EpCAM_SE_DP,
    ...
    geom_pwc(method = "wilcox_test", label = "p = {p}") +
    scale_fill_tron() +
    scale_x_discrete(labels = c("EpCAM+PSMA+","EpCAM+PSMA-")) +
...
rm(long_niSortFilt_EpCAM_SE_DP)
...
# Visualizing PSMA channel between response groups broken down to marker status
...
rm(long_niSortFilt_PSMA_SP_DP)


# Removing unnecessary variables from this section
rm(MWUt_NRvR_EpCAM, MWUt_NRvR_PSMA)
